...
- 屏幕右边缘有 5 个**悬浮球**（今日任务/金钱/日历/项目/知识库），可拖拽换位（位置自动记住），点击在球旁弹出对应功能的**快捷弹窗**；再点一次同球或弹窗右上角关闭。
...
- X11 下悬浮球的点击/拖拽由壳在 X11 层原生接管（监听 XI2 raw 事件，命中球窗几何直接触发弹窗/拖拽），绕过了 xrdp 等环境 wry webview 丢失鼠标按键的已知问题；弹窗头部的「关闭/打开主窗口」两个按钮区同样由壳原生接管，弹窗失焦自动收起。弹窗内部仍走 webview，在这类环境下弹窗内按钮可能无法点击（弹窗内容仅供查看），正常桌面环境不受影响。
